...
class DB_Manager:
    ...
    # Size of cols_info must be equal (table_columns - 1), cause cod_prod is self generated by the db
    ...
    def add_table(self, table_name:str, columns:int, cols_info:list[tuple[str, str]]):
        ...
        query += "cod_produto SERIAL PRIMARY KEY, "

        for i in range(columns-1):
            # Making cod_produto the primary key and SERIAL
            #if cols_info[i][0] == "cod_produto":
            #    query += f"{cols_info[i][0]} {cols_info[i][1]} PRIMARY KEY, "
            #    continue
